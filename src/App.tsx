...
function App() {
  const { habits, checkIns, notes, createHabit, updateHabit, deleteHabit, setStatus, archiveHabit, restoreHabit, setNote } = useHabits();
  ...
  const todaySummary = getDailyConsistency(today, fixedHabits, checkIns);
  const daysRemaining = 6 - new Date().getDay();
  const streaks = getStreaks(habits, checkIns);
  const monthly = getMonthlyStats(month, habits, checkIns);
  const categoryStats = getCategoryConsistency(month, habits, checkIns);
  ...
  return <main className="dashboard">
    ...
    {selectedDate && <DayDetails date={selectedDate} summary={getDailyConsistency(selectedDate, habits, checkIns)} note={notes.find((note) => note.date === selectedDate)?.text} onSaveNote={(text) => setNote(selectedDate, text)} onClose={() => setSelectedDate(null)} />}
  </main>;
}
export default App;
